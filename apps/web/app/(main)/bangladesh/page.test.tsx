...
import BangladeshPage from './page';
...
describe('BangladeshPage', () => {
  const TEST_URL = 'https://powerbi.example.test/view?r=abc';
  const TITLE = 'Bangladesh Procurement Analysis SPECUS Dashboard';

  beforeEach(() => {
    vi.stubEnv('NEXT_PUBLIC_BANGLADESH_DASHBOARD_URL', TEST_URL);
  });
  ...
  it('triggers notFound when the dashboard URL env var is unset', async () => {
    vi.stubEnv('NEXT_PUBLIC_BANGLADESH_DASHBOARD_URL', '');
    const { notFound } = await import('next/navigation');

    expect(() => render(<BangladeshPage />)).toThrow('NEXT_NOT_FOUND');
